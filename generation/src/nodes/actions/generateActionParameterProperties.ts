...
function generateMessageCollection(
	f: GeneratedFile,
	field: DescField,
	indentation: number,
	parents: string = '',
...
${(field.proto.proto3Optional
	...
	: [
			field.message?.oneofs?.length
				? field.message.oneofs
						.map((oneof: DescOneof) => `${idt}      ${oneof.localName}Select: 'undefined',\n`)
						.join('')
				: '',
			field.message?.fields
				.map((subField: DescField) =>
					subField.proto.proto3Optional
						? ''
						: `${idt}      ${subField.jsonName}: ${getDefaultType(subField)},\n`,
				)
				.join(''),
		]
)
...
function generateActionPropertiesJsonRecursive(
	destinationFile: GeneratedFile,
	field: DescField,
	indentation: number,
	parents: string = '',
	oneofNames: string[],
...
${idt}        values: [`;
		generateActionPropertiesJsonRecursive(
			destinationFile,
			field,
			indentation + 4,
			parents,
...
${idt}  },`;
		for (const oneofField of field.oneof.fields) {
			generateActionPropertiesJsonRecursive(
				...
				oneofField,
				indentation,
				parents + ' - ' + capitalize(oneofField.jsonName),
				oneofNames,
				true,
			);
		}
		...
		for (const subField of field.message.fields) {
			...
			generateActionPropertiesJsonRecursive(
				...
				subField,
				indentation + 2,
				parents + '| ' + capitalize(subField.jsonName),
				oneofNames,
			);
		}
		destinationFile.print`${idt}    ],
...
export function generateActionParameterProperties(destinationFIle: GeneratedFile, method: DescMethod): void {
	destinationFIle.print`
const properties: INodeProperties[] = [`;
	if (method.methodKind === 'client_streaming' || method.methodKind === 'bidi_streaming') {
		throw new Error(
			`#--# GENERATION ERROR: generateActionPropertiesJson: Client streaming and bidirectional streaming not supported: "${method.parent.name}"`,
		);
	}
