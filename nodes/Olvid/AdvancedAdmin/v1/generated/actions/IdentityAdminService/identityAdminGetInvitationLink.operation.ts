...
const displayOptions = {
  show: {
    ...
    operation: ['IdentityAdminGetInvitationLink'],
  },
};

export const identityAdminGetInvitationLinkProperties = updateDisplayOptions(displayOptions, properties);

export async function identityAdminGetInvitationLink(this: IExecuteFunctions, index: number, client: OlvidAdminClient): Promise<INodeExecutionData[]> {
    const identityId: bigint = BigInt(this.getNodeParameter('identityId', index) as number);
    const response: admin.IdentityAdminGetInvitationLinkResponse = await client.adminStubs.identityAdminStub.identityAdminGetInvitationLink({identityId});
