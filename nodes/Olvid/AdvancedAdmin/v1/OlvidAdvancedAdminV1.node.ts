...
import {buildListResponseMessage, buildResponseMessage} from "../../Advanced/v1/convertProtobufToN8n";
    ...
    async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
			...
			for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
				...
				if (stubFunction === undefined) {
					throw new NodeOperationError(this.getNode(), `Invalid action resource / operation combination for stub function: ${resourceName} & ${operationName}`);
				}
				...
				if (responseSchema === undefined) {
					throw new NodeOperationError(this.getNode(), `Invalid action resource / operation combination for response schema: ${resourceName} & ${operationName}`);
				}
				...
				try {
					...
					const executionDataWithMetadata = this.helpers.constructExecutionMetaData(
						responseData,
