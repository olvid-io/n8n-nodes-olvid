...
export function buildDryRunMessage(messageDesc: DescMessage, returnJsonArray: (jsonData: IDataObject | IDataObject[]) => INodeExecutionData[]): INodeExecutionData[] {
...
function recursivelyBuildDryRunMessage(field: DescField, options:{ignoreOneOf?: boolean, ignoreList?: boolean}): IDataObject {
	const dataObject: IDataObject = {};

	if (field.oneof && !options.ignoreOneOf) {
		Object.assign(dataObject, recursivelyBuildDryRunMessage(field, {ignoreOneOf: true, }));
	}
	else if (field.fieldKind === "list" && !options.ignoreList) {
		dataObject[field.localName] = [recursivelyBuildDryRunMessage(field, {ignoreList: true})].map((e: IDataObject) => { return e[field.localName] });
	}
	else if (field.message) {
		const subDataObject: IDataObject = {};
		field.message.fields.forEach((subField) => {Object.assign(subDataObject, recursivelyBuildDryRunMessage(subField, {}))})
