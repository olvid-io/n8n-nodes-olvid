...
const displayOptions = {
  show: {
    ...
    operation: ['GroupLeave'],
  },
};

export const groupLeaveProperties = updateDisplayOptions(displayOptions, properties);

export async function groupLeave(this: IExecuteFunctions, index: number, client: OlvidClient): Promise<INodeExecutionData[]> {
    const groupId: bigint = BigInt(this.getNodeParameter('groupId', index) as number);
    const response: commands.GroupLeaveResponse = await client.stubs.groupCommandStub.groupLeave({groupId});
