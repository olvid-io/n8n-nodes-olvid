...
const displayOptions = {
  show: {
    ...
    operation: ['GroupUnsetPhoto'],
  },
};

export const groupUnsetPhotoProperties = updateDisplayOptions(displayOptions, properties);

export async function groupUnsetPhoto(this: IExecuteFunctions, index: number, client: OlvidClient): Promise<INodeExecutionData[]> {
    const groupId: bigint = BigInt(this.getNodeParameter('groupId', index) as number);
    const response: commands.GroupUnsetPhotoResponse = await client.stubs.groupCommandStub.groupUnsetPhoto({groupId});
