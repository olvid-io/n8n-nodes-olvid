...
const displayOptions = {
  show: {
    ...
    operation: ['GroupDisband'],
  },
};

export const groupDisbandProperties = updateDisplayOptions(displayOptions, properties);

export async function groupDisband(this: IExecuteFunctions, index: number, client: OlvidClient): Promise<INodeExecutionData[]> {
    const groupId: bigint = BigInt(this.getNodeParameter('groupId', index) as number);
    const response: commands.GroupDisbandResponse = await client.stubs.groupCommandStub.groupDisband({groupId});
