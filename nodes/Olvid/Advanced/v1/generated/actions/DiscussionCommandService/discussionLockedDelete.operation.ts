...
const displayOptions = {
  show: {
    ...
    operation: ['DiscussionLockedDelete'],
  },
};

export const discussionLockedDeleteProperties = updateDisplayOptions(displayOptions, properties);

export async function discussionLockedDelete(this: IExecuteFunctions, index: number, client: OlvidClient): Promise<INodeExecutionData[]> {
    const discussionId: bigint = BigInt(this.getNodeParameter('discussionId', index) as number);
    await client.stubs.discussionCommandStub.discussionLockedDelete({discussionId});
