...
import { buildListResponseMessage, buildResponseMessage } from './convertProtobufToN8n';
    ...
    async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
			...
			for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
				...
				if (stubFunction === undefined) {
					throw new NodeOperationError(this.getNode(), `Invalid action resource / operation combination for stub function: ${resourceName} & ${operationName} (${stubName} / ${stubMethodName})`);
				}
				...
				if (responseSchema === undefined) {
					throw new NodeOperationError(this.getNode(), `Invalid action resource / operation combination for response schema: ${resourceName} & ${operationName} (${schemaName})`);
				}
				...
				try {
					...
					if (operationName === 'MessageSendWithAttachments') {
							responseData = await handleMessageSendWithAttachments(client, requestShape, this, itemIndex);
					}
							...
							responseData = await handleGroupSetPhoto(client, requestShape, this, itemIndex);
					}
					else if (operationName === 'IdentitySetPhoto') {
							responseData = await handleIdentitySetPhoto(client, requestShape, this, itemIndex);
					}
					// handle normal cases
					else {
							...
							if (typeof ret[Symbol.asyncIterator] === 'function') {
									for await (const message of ret) {
										responseData.push(...buildListResponseMessage(message, responseSchema, this.helpers.returnJsonArray));
									}
							}
							else {
									responseData = buildResponseMessage(ret, responseSchema, this.helpers.returnJsonArray);
							}
					}
 ...
 */
async function handleMessageSendWithAttachments(client: OlvidClient, requestShape: any, node: IExecuteFunctions, itemIndex: number): Promise<INodeExecutionData[]> {
...
async function handleAttachmentDownload(client: OlvidClient, requestShape: any, node: IExecuteFunctions): Promise<INodeExecutionData[]> {
	const attachment: datatypes.Attachment = await client.attachmentGet(requestShape);
...
async function handleGroupSetPhoto(client: OlvidClient, requestShape: INodeParameters, node: IExecuteFunctions, itemIndex: number): Promise<INodeExecutionData[]> {
	const groupId: bigint = BigInt(node.getNodeParameter('groupId', itemIndex) as number);
	const binaryPropertyName: string = node.getNodeParameter('binaryPropertyName', itemIndex) as string;
	const binaryData = node.helpers.assertBinaryData(itemIndex, binaryPropertyName);
	const payload = await node.helpers.getBinaryDataBuffer(itemIndex, binaryPropertyName);
	...
	return buildResponseMessage(response, commands.GroupSetPhotoResponseSchema, node.helpers.returnJsonArray);
}

async function handleIdentitySetPhoto(client: OlvidClient, requestShape: INodeParameters, node: IExecuteFunctions, itemIndex: number): Promise<INodeExecutionData[]> {
	const binaryPropertyName: string = node.getNodeParameter('binaryPropertyName', itemIndex) as string;
	const binaryData = node.helpers.assertBinaryData(itemIndex, binaryPropertyName);
	const payload = await node.helpers.getBinaryDataBuffer(itemIndex, binaryPropertyName);
	...
	const response: commands.IdentitySetPhotoResponse = await client.stubs.identityCommandStub.identitySetPhoto(requestStream());
	return buildResponseMessage(response, commands.IdentitySetPhotoResponseSchema, node.helpers.returnJsonArray);
}

function getStubName(resourceName: string): string {
	const decapitalizedName = resourceName.charAt(0).toLowerCase() + resourceName.slice(1);
