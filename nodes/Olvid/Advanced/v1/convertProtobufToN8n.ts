...
export function buildResponseMessage(
	...
	for (const field of messageDesc.fields) {
		Object.assign(
			dataObject,
			recursivelyBuildResponseMessage(originalMessage, field),
		);
	}
...
export function buildListResponseMessage(
	...
	const originalExecutionData: INodeExecutionData[] = buildResponseMessage(
		originalMessage,
		messageDesc,
		returnJsonArray,
	);
	...
	messageDesc.fields.forEach((field) => {
		if (field.fieldKind === 'list') {
			...
			if (!field.message && !field.localName.endsWith('s')) {
				console.warn(
					`buildListResponseMessage: ${messageDesc}.${field.localName}: name is not valid and you might extend generation code`,
				);
			}
			(originalDataObject[field.localName] as IDataObject[]).forEach(
				(e: IDataObject) => {
					const newKey = field.message
						? decapitalize(
								field.message.typeName.split('.')[
									field.message.typeName.split('.').length - 1
								],
							)
						: field.localName.substring(0, field.localName.length - 1);
...
function recursivelyBuildResponseMessage(
	originalMessage: any,
	field: DescField,
	ignoreOneOf: boolean = false,
	ignoreList: boolean = false,
): IDataObject {
	const dataObject: IDataObject = {};

	if (field.oneof && !ignoreOneOf) {
		if (
			originalMessage[field.oneof.localName] &&
			originalMessage[field.oneof.localName]['case'] === field.localName
		) {
			...
			fakeOriginalMessage[field.localName] =
				originalMessage[field.oneof.localName]['value'];
			Object.assign(
				dataObject,
				recursivelyBuildResponseMessage(fakeOriginalMessage, field, true),
			);
		}
		...
		if (originalMessage[field.localName]) {
			...
			originalMessage[field.localName].forEach((e: any) => {
				...
				elementList.push(
					recursivelyBuildResponseMessage(
						fakeOriginalMessage,
						field,
						false,
						true,
			...
			dataObject[field.localName] = elementList.map((e: IDataObject) => {
		...
		if (originalMessage[field.localName]) {
			...
			field.message.fields.forEach((subField) => {
				Object.assign(
					subDataObject,
					recursivelyBuildResponseMessage(
						originalMessage[field.localName],
						subField,
					),
				);
			});
	...
	// handle bytes fields
		...
		switch (ScalarProto_TypeJson[field.scalar]) {
			...
			case 'boolean': {
				dataObject[field.localName] = originalMessage[
					field.localName
				] as boolean;
				break;
			}
			...
			case 'bytes': {
				dataObject[field.localName] = base64Encode(
					originalMessage[field.localName],
				).toString();
				break;
			}
