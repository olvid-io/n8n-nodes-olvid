		...
		for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
			...
			try {
				...
				if (resourceName === 'message' && operationName === 'send') {
					responseData = await handleMessageSend(
						client,
						requestShape,
						this,
						itemIndex,
					...
					operationName === 'sendAndWait'
				) {
					responseData = await handleMessageSendAndWait(
						client,
						requestShape,
						this,
						itemIndex,
				...
				// handle normal cases
					...
					throw new ApplicationError(
						`Invalid resource and/or operation: ${resourceName} - ${operationName}`,
					);
				}
...
async function handleMessageSend(
	...
	const additionalFields = resolveParameterFunction(
		'additionalFields',
	) as IDataObject;
	...
	requestShape['ephemerality']['existenceDuration'] = getDurationInSeconds(
		additionalFields,
		'existenceDuration',
	);
	requestShape['ephemerality']['visibilityDuration'] = getDurationInSeconds(
		additionalFields,
		'visibilityDuration',
	);
...
async function handleMessageSendAndWait(
	client: OlvidClient,
	requestShape: any,
	node: IExecuteFunctions,
	itemIndex: number,
...
async function handleAttachmentDownload(
	...
	for await (const attachment of client.attachmentList({
		filter: create(datatypes.AttachmentFilterSchema, {
			messageId: requestShape['messageId'],
		}),
	})) {
